...
- [Development guidelines](#development-guidelines)
    - [General avianitecoin](#general-avianitecoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
avianitecoin uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play AVIANs (tAVIAN)" on the test network, if you
...
avianitecoin is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/avianitecoinvps-project/avianitecoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_avianitecoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_avianitecoin --log_level=test_suite
$ valgrind -v --leak-check=full src/avianitecoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_avianitecoin.coverage/index.html`.
```
...
- BitcoinMiner : Generates AVIANs (if wallet is enabled).
...
However, in open source software such as avianitecoin, where everyone uses
...
pay attention to for reviewers of avianitecoin code.

General avianitecoin
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: avianitecoin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against avianitecoin so that they can be integrated
...
file descriptors it will cause problems with avianitecoin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:avianitecoinvps-project/avianitecoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `avianitecoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `avianitecoin-cli` and the GUI debug console use this table to determine how to
