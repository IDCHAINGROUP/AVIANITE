...
*Setup instructions for a gitian build of avianitecoin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the avianitecoin
...
to the avianitecoin GitHub release page.
...
- [Building avianitecoin](#building-avianitecoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/avianitecoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/avianitecoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/avianitecoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/avianitecoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/avianitecoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/avianitecoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/avianitecoinvps-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@avianitecoin.trade) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/avianitecoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/avianitecoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/avianitecoin-osx-*-build.assert
```
