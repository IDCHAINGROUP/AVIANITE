...
bool CAVIANStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CAVIANStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CAVIANStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CAVIANStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CAVIANStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CAVIANStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CAVIANStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CAVIANStake::GetUniqueness() const
{
    //The unique identifier for a AVIAN stake is the outpoint
...
CBlockIndex* CAVIANStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CAVIANStake::ContextCheck(int nHeight, uint32_t nTime)
{
