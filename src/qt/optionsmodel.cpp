...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::AVIAN);
    ...
    if (settings.contains("nAnonymizeavianitecoinAmount") || reset)
        SoftSetArg("-anonymizeavianitecoinamount", settings.value("nAnonymizeavianitecoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in avianitecoin.cpp during shotdown to also remove the window positions
