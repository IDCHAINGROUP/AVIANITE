...
#include "qt/avianitecoin/dashboardwidget.h"
#include "qt/avianitecoin/forms/ui_dashboardwidget.h"
#include "qt/avianitecoin/sendconfirmdialog.h"
#include "qt/avianitecoin/txrow.h"
...
DashboardWidget::DashboardWidget(avianitecoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareAVIAN, "square-chart-avianitecoin");
    setCssProperty(ui->labelAVIAN, "text-chart-avianitecoin");
    ...
    setCssProperty(ui->labelAmountAVIAN, "text-stake-avianitecoin-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &avianitecoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &avianitecoinGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &avianitecoinGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        qreal avianitecoin = 0;
        if (chartData->amountsByCache.contains(num)) {
            std::pair <qint64, qint64> pair = chartData->amountsByCache[num];
            avianitecoin = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalAVIAN += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesAVIAN.append(avianitecoin);

        int max = std::max(avianitecoin, avianitecoin);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesAVIAN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalAVIAN > 0) {
        setCssProperty(ui->labelAmountAVIAN, "text-stake-avianitecoin");
    } else {
        setCssProperty(ui->labelAmountAVIAN, "text-stake-avianitecoin-disable");
    }
    forceUpdateStyle({ui->labelAmountAVIAN});
    ui->labelAmountAVIAN->setText(GUIUtil::formatBalance(chartData->totalAVIAN, nDisplayUnit));
