...
#include "qt/avianitecoin/forms/ui_snackbar.h"
#include "qt/avianitecoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(avianitecoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &avianitecoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
