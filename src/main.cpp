...
#if defined(NDEBUG)
#error "avianitecoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uavianitecoin) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateAVIANSupply(int nHeightStart)
{
	AssertLockHeld(cs_main);
	...
	uiInterface.ShowProgress(_("Recalculating AVIAN supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating AVIAN supply..."), percent);
		}
...
CAmount GetDevPayment(int nHeight, CAmount blockValue)
{
    ...
    if (nHeight <= 1000) { 
        ...
        nDevPayment = 10.00 * COIN;
    } else if (nHeight <= 370000) { 
        nDevPayment = 750 * COIN;
    } else if (nHeight <= 385000) { 
        nDevPayment = 900 * COIN;
    } else if (nHeight <= 390000) { 
        nDevPayment = 100900 * COIN;
    } else if (nHeight <= 400000) { 
        nDevPayment = 1000 * COIN;
    } else if (nHeight <= 410000) { 
        nDevPayment = 400 * COIN;
    } else if (nHeight <= 420000) { 
        nDevPayment = 300 * COIN;
    } else if (nHeight <= 430000) { 
        nDevPayment = 250 * COIN;
    } else if (nHeight <= 440000) { 
        nDevPayment = 200 * COIN;
    } else if (nHeight <= 450000) { 
        nDevPayment = 150 * COIN;
    } else if (nHeight <= 460000) { 
        nDevPayment = 100 * COIN;
    } else { 
        nDevPayment = 300 * COIN;
    }
...
{
    util::ThreadRename("avianitecoin-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> avianitecoinInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                avianitecoinInputs.push_back(stakeIn);
        }
        const bool hasAVIANInputs = !avianitecoinInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasAVIANInputs) {
                    ...
                    for (const CTxIn& avianitecoinIn : avianitecoinInputs)
                        if(avianitecoinIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasAVIANInputs) {
                            for (const CTxIn& stakeIn : avianitecoinInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // avianitecoin: We use certain sporks during IBD, so check to see if they are
